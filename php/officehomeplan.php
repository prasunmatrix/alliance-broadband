...
//if(isset($_POST['form_key']) || $formKey->validate()):
  ...
  if(isset($_POST['frmsubmitOffice'])){
    ...
    //if($response->success == 1) {
        ...
        if(isset($_POST['mail_subject']) && (trim($_POST['mail_subject'])!= '')){
            ...
            $mail_subject = 'A new home office plan submission for Alliance Broadband';
        }

        $number_of_device = '';
        $city_office_from_home = '';
        $plan_office_from_home = '';
        $address_from='';
        $address = '';
        $landmark='';
        $pincode = '';
        ...
        if(isset($_POST['city_office_from_home']) && (trim($_POST['city_office_from_home'])!= '')){
          $city_office_from_home = trim($_POST['city_office_from_home']); 
        }
        if(isset($_POST['plan_office_from_home']) && (trim($_POST['plan_office_from_home'])!= '')){
          $plan_office_from_home = trim($_POST['plan_office_from_home']); 
        }
        ...
        if($address_from == 'Use Current Location') {
          $address = $_POST['popupform_2_a'] ?? '';
          $landmark = $_POST['popupform_2_l'] ?? '';
          $pincode = $_POST['popupform_2_p'] ?? '';
        }
        ...
        //end
        $sql = "INSERT INTO `wp_homeofficeplan` (`number_devices`,`city_id`,`plan_id`,`address_from`,`address`,`landmark`,`pincode`,`phone_no`,`company_name`,`uemail`) VALUES ('$number_of_device','$city_office_from_home','$plan_office_from_home','$address_from','$address','$landmark','$pincode','$phone_no','$company_name','$uemail')";
        mysqli_query($conn, $sql);
        $lastid = mysqli_insert_id($conn);

        global $wpdb;

        $cityMasterExists = $wpdb->get_results("SELECT city_name from wp_city_master where id=$city_office_from_home limit 1");
        $planMasterExists = $wpdb->get_results("SELECT * from wp_plan_master where id=$plan_office_from_home limit 1");

        // Starts
        $curl = curl_init();
        curl_setopt_array($curl, array(
          CURLOPT_URL => 'https://alliancebroadband.co.in/alliance-spreadsheet/office-plan.php',
          ...
          CURLOPT_POSTFIELDS => array(
            ...
            'city_office_from_home' =>  $cityMasterExists[0]->city_name ?? '',
            'plan_office_from_home' =>  ($planMasterExists[0]->plan_name.'-'.$planMasterExists[0]->speed) ?? '',
              ...
              <tr>
                ...
                <center>
                    ...
                    <table cellspacing="0" cellpadding="0" width="100%" style="font-size:14px;"> 
                      <tr>
                        <td style="text-align:left; padding: 38px 0;"><span class="lead-txt"> Hi '.$_POST['company_name'].', <br/>
                          <br/>
        ...
        <div class="wrapper">
          ...
          if (!$mail_1->send()) {
            ...
            header("Location: ../thank-you-home-plan-business/?id=".$lastid);
            // echo '<script>
            // 	$(document).ready(function () {
            //     $("#feed_suc").show();
            //   });</script>';
